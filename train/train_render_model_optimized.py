import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "true"
current_dir = os.path.dirname(os.path.abspath(__file__))
import sys
sys.path.append(os.path.join(current_dir, ".."))
from talkingface.models.common.Discriminator import Discriminator
from talkingface.models.common.VGG19 import Vgg19
from talkingface.models.DINet import DINet_five_Ref
from talkingface.util.utils import GANLoss,get_scheduler, update_learning_rate
from talkingface.config.config_optimized import DINetTrainingOptions
from torch.utils.tensorboard import SummaryWriter
from talkingface.util.log_board import log
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
import random
import numpy as np
import os
import sys
import torch.nn.functional as F
import cv2
from talkingface.data.few_shot_dataset_optimized import Few_Shot_Dataset_Optimized, data_preparation

def Tensor2img(tensor_, channel_index):
    """将张量转换为图像"""
    frame = tensor_[channel_index:channel_index + 3, :, :].detach().squeeze(0).cpu().float().numpy()
    frame = np.transpose(frame, (1, 2, 0)) * 255.0
    frame = frame.clip(0, 255)
    return frame.astype(np.uint8)

if __name__ == "__main__":
    '''
    优化版本的数字人训练代码 - 支持128分辨率训练以提高唇形清晰度
    '''
    # 加载优化配置
    opt = DINetTrainingOptions().parse_args()
    n_ref = 5
    opt.source_channel = 3 * 2
    opt.target_channel = 3
    opt.ref_channel = n_ref * 3 * 2
    
    # 使用配置文件中的优化参数
    print(f"Training with optimized parameters:")
    print(f"- Mouth region size: {opt.mouth_region_size}px")
    print(f"- Batch size: {opt.batch_size}")
    print(f"- Learning rates: G={opt.lr_g}, D={opt.lr_d}")
    print(f"- Loss weights: perception={opt.lamb_perception}, pixel={opt.lamb_pixel}")
    
    opt.resume = False
    opt.resume_path = None

    # 设置随机种子
    random.seed(opt.seed)
    np.random.seed(opt.seed)
    torch.cuda.manual_seed(opt.seed)
    torch.manual_seed(opt.seed)
    
    # 检查CUDA可用性
    if not torch.cuda.is_available():
        print("Warning: CUDA not available, using CPU training (will be very slow)")
        device = torch.device('cpu')
    else:
        device = torch.device('cuda')
        print(f"Using GPU: {torch.cuda.get_device_name()}")

    # 准备训练数据
    video_list = []
    path_ = opt.train_data
    if os.path.exists(path_):
        video_list += [os.path.join(path_, i) for i in os.listdir(path_)]
    else:
        print(f"Warning: Training data path {path_} does not exist")
        
    print("video_selected final: ", len(video_list))
    video_list.sort()
    
    # 使用优化版本的数据准备和数据集
    train_dict_info = data_preparation(video_list[:])
    train_set = Few_Shot_Dataset_Optimized(
        train_dict_info, 
        n_ref=n_ref, 
        is_train=True, 
        target_size=opt.mouth_region_size  # 使用配置中的分辨率
    )
    training_data_loader = DataLoader(
        dataset=train_set, 
        num_workers=0, 
        batch_size=opt.batch_size, 
        shuffle=True
    )
    
    train_log_path = "train_log_optimized.txt"
    train_data_length = len(training_data_loader)
    print(f"Training data loader created with {train_data_length} batches")

    # 初始化网络
    net_g = DINet_five_Ref(opt.source_channel, opt.ref_channel).to(device)
    net_d = Discriminator(opt.target_channel, opt.D_block_expansion, opt.D_num_blocks, opt.D_max_features).to(device)
    net_vgg = Vgg19().to(device)
    
    print(f"Networks initialized on {device}")

    # 初始化优化器
    optimizer_g = optim.Adam(net_g.parameters(), lr=opt.lr_g, betas=(0.5, 0.999))
    optimizer_d = optim.Adam(net_d.parameters(), lr=opt.lr_d, betas=(0.5, 0.999))

    # 恢复训练（如果需要）
    if opt.resume and opt.resume_path and os.path.exists(opt.resume_path):
        print('Loading checkpoint {}'.format(opt.resume_path))
        checkpoint = torch.load(opt.resume_path, map_location=device)
        opt.start_epoch = checkpoint['epoch']
        net_g_static = checkpoint['state_dict']['net_g']
        net_g.load_state_dict(net_g_static)
        net_d.load_state_dict(checkpoint['state_dict']['net_d'])
        optimizer_g.load_state_dict(checkpoint['optimizer']['net_g'])
        optimizer_d.load_state_dict(checkpoint['optimizer']['net_d'])
        print(f'Resumed from epoch {opt.start_epoch}')

    # 损失函数
    criterionGAN = GANLoss().to(device)
    criterionL1 = nn.L1Loss().to(device)

    # 学习率调度器
    net_g_scheduler = get_scheduler(optimizer_g, opt.non_decay, opt.decay)
    net_d_scheduler = get_scheduler(optimizer_d, opt.non_decay, opt.decay)

    # TensorBoard日志
    train_log_path = os.path.join("checkpoint/{}/log".format("DiNet_five_ref_optimized"), "train")
    os.makedirs(train_log_path, exist_ok=True)
    train_logger = SummaryWriter(train_log_path)
    
    print(f"Starting optimized training for {opt.non_decay + opt.decay} epochs...")

    # 训练循环
    for epoch in range(opt.start_epoch, opt.non_decay + opt.decay + 1):
        net_g.train()
        net_d.train()
        
        avg_loss_g_perception = 0
        avg_Loss_DI = 0
        avg_Loss_GI = 0
        
        for iteration, data in enumerate(training_data_loader):
            try:
                source_tensor, ref_tensor, target_tensor = data
                source_tensor = source_tensor.float().to(device)
                ref_tensor = ref_tensor.float().to(device)
                target_tensor = target_tensor.float().to(device)

                source_tensor, source_prompt_tensor = source_tensor[:, :3], source_tensor[:, 3:]

                # 生成器前向传播
                fake_out = net_g(source_tensor, source_prompt_tensor, ref_tensor)

                # 多尺度处理
                fake_out_half = F.avg_pool2d(fake_out, 3, 2, 1, count_include_pad=False)
                target_tensor_half = F.interpolate(target_tensor, scale_factor=0.5, mode='bilinear', align_corners=False)

                # 训练判别器
                optimizer_d.zero_grad()

                _, pred_fake_d = net_d(fake_out.detach())
                loss_d_fake = criterionGAN(pred_fake_d, False)

                _, pred_real_d = net_d(target_tensor)
                loss_d_real = criterionGAN(pred_real_d, True)

                loss_dI = (loss_d_fake + loss_d_real) * 0.5
                loss_dI.backward()
                optimizer_d.step()

                # 训练生成器
                optimizer_g.zero_grad()
                _, pred_fake_dI = net_d(fake_out)

                # 感知损失（使用优化的权重）
                perception_real = net_vgg(target_tensor)
                perception_fake = net_vgg(fake_out)
                perception_real_half = net_vgg(target_tensor_half)
                perception_fake_half = net_vgg(fake_out_half)
                
                loss_g_perception = 0
                for i in range(len(perception_real)):
                    loss_g_perception += criterionL1(perception_fake[i], perception_real[i])
                    loss_g_perception += criterionL1(perception_fake_half[i], perception_real_half[i])
                loss_g_perception = (loss_g_perception / (len(perception_real) * 2)) * opt.lamb_perception

                # 对抗损失
                loss_g_dI = criterionGAN(pred_fake_dI, True)

                # 像素级损失（新增）
                loss_g_pixel = criterionL1(fake_out, target_tensor) * opt.lamb_pixel

                # 总生成器损失
                loss_g = loss_g_perception + loss_g_dI + loss_g_pixel
                loss_g.backward()
                optimizer_g.step()

                # 记录损失
                message = "===> Epoch[{}]({}/{}): Loss_DI: {:.4f} Loss_GI: {:.4f} Loss_perception: {:.4f} Loss_pixel: {:.4f} lr_g = {:.7f} lr_d = {:.7f}".format(
                    epoch, iteration, len(training_data_loader), 
                    float(loss_dI), float(loss_g_dI), float(loss_g_perception), float(loss_g_pixel),
                    optimizer_g.param_groups[0]['lr'], optimizer_d.param_groups[0]['lr']
                )
                print(message)

                # 定期保存训练样本
                if iteration % 200 == 0:
                    with torch.no_grad():
                        inference_out = fake_out * 255
                        inference_out = inference_out[0].cpu().permute(1, 2, 0).float().detach().numpy().astype(np.uint8)
                        inference_in = (target_tensor[0, :3] * 255).cpu().permute(1, 2, 0).float().detach().numpy().astype(np.uint8)
                        inference_in_prompt = (source_prompt_tensor[0, :3] * 255).cpu().permute(1, 2, 0).float().detach().numpy().astype(np.uint8)
                        
                        frame2 = Tensor2img(ref_tensor[0], 0)
                        frame3 = Tensor2img(ref_tensor[0], 3)
                        inference_out_combined = np.concatenate([inference_in, inference_in_prompt, inference_out, frame2, frame3], axis=1)
                        inference_out_combined = cv2.cvtColor(inference_out_combined, cv2.COLOR_RGB2BGR)

                        log(train_logger, fig=inference_out_combined, tag="Training_Optimized/epoch_{}_{}_{}".format(epoch, iteration, opt.mouth_region_size))

                    # TensorBoard记录
                    real_iteration = epoch * len(training_data_loader) + iteration
                    losses = [loss_dI.item(), loss_g_perception.item(), loss_g_dI.item(), loss_g_pixel.item()]
                    train_logger.add_scalar("Loss/loss_dI", losses[0], real_iteration)
                    train_logger.add_scalar("Loss/loss_g_perception", losses[1], real_iteration)
                    train_logger.add_scalar("Loss/loss_g_dI", losses[2], real_iteration)
                    train_logger.add_scalar("Loss/loss_g_pixel", losses[3], real_iteration)

                avg_loss_g_perception += loss_g_perception.item()
                avg_Loss_DI += loss_dI.item()
                avg_Loss_GI += loss_g_dI.item()
                
            except Exception as e:
                print(f"Error in iteration {iteration}: {str(e)}")
                continue

        # 记录每个epoch的平均损失
        train_logger.add_scalar("Loss/epoch_g_perception", avg_loss_g_perception / len(training_data_loader), epoch)
        train_logger.add_scalar("Loss/epoch_DI", avg_Loss_DI / len(training_data_loader), epoch)
        train_logger.add_scalar("Loss/epoch_GI", avg_Loss_GI / len(training_data_loader), epoch)
        
        # 更新学习率
        update_learning_rate(net_g_scheduler, optimizer_g)
        update_learning_rate(net_d_scheduler, optimizer_d)

        # 保存检查点
        if epoch % opt.checkpoint == 0:
            if not os.path.exists(opt.result_path):
                os.makedirs(opt.result_path, exist_ok=True)
            model_out_path = os.path.join(opt.result_path, 'epoch_{}_{}.pth'.format(epoch, opt.mouth_region_size))
            states = {
                'epoch': epoch + 1,
                'state_dict': {'net_g': net_g.state_dict(), 'net_d': net_d.state_dict()},
                'optimizer': {'net_g': optimizer_g.state_dict(), 'net_d': optimizer_d.state_dict()},
                'config': {
                    'mouth_region_size': opt.mouth_region_size,
                    'batch_size': opt.batch_size,
                    'lamb_perception': opt.lamb_perception,
                    'lamb_pixel': opt.lamb_pixel
                }
            }
            torch.save(states, model_out_path)
            print(f"Optimized checkpoint saved to {model_out_path} (epoch {epoch})")

    print("Optimized training completed!")
    train_logger.close()