    ...
    navigator.mediaDevices
      ...
      .then(stream => {
        ...
        scriptProcessor.onaudioprocess = event => {
          ...
          sendWebSocketMessage(floatTo16BitPCM(resampledData));
        };
      ...
      .catch(error => {
        ...
        alert('需要麦克风权限才能使用语音功能，请在浏览器设置中允许麦克风访问。');
      });
  }
}
...
overlayButton.addEventListener('click', async () => {
  ...
  switchInputMode(currentInputModelIsVoice)
...
function createWebSocketConnection() {
  try {
    socket = new WebSocket('wss://2dhuman.lkz.fit/recognition?isSendConfig=true&isFree=true');
    //socket = new WebSocket('ws://localhost:19465/recognition?isSendConfig=true&isFree=true');
    
    socket.addEventListener('open', (event) => {
      console.log('WebSocket连接已建立');
      socketReconnectAttempts = 0; // 重置重连计数器
...
回复字数限制在50字以内`;
  ...
  const jsonString = JSON.stringify(data);
    ...
    socket.addEventListener('close', (event) => {
      ...
      // 如果不是正常关闭且重连次数未达到上限，则尝试重连
      if (event.code !== 1000 && socketReconnectAttempts < maxReconnectAttempts) {
        socketReconnectAttempts++;
        console.log(`尝试重连WebSocket (${socketReconnectAttempts}/${maxReconnectAttempts})...`);
        setTimeout(() => {
          createWebSocketConnection();
        }, 2000 * socketReconnectAttempts); // 递增延迟重连
      } else if (socketReconnectAttempts >= maxReconnectAttempts) {
...
function PlayWav() {
  ...
  try {
    ...
    // Web Audio API播放（备用方案）
    audioContext.decodeAudioData(audioUint8Array.slice(), function (audioBuffer) {
      // 创建BufferSource节点
